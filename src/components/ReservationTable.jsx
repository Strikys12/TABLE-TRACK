                ...
                <tbody className="divide-y divide-orange-950/10 text-sm">
                    {reservations.map((res) => (
                        <tr key={res.id} className="hover:bg-white/[0.02]">
                            ...
                            <td className="p-4">{res.fechaHora}</td>
                            <td className="p-4">{statusBadgeRenderer(res.estado)}</td>
                            <td className="p-4 flex gap-2 justify-center">
                                {res.estado !== 'Finalizada' && (
                                    <button onClick={() => onComplete(res)} className="text-xs bg-green-950/40 text-green-400 px-2 py-1 rounded">✓</button>
                                )}
                                <button onClick={() => onEdit(res)} className="text-xs bg-blue-950/40 text-blue-400 px-2 py-1 rounded">Edit</button>
                                <button onClick={() => onDelete(res.id)} className="text-xs bg-red-950/40 text-red-400 px-2 py-1 rounded">X</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
